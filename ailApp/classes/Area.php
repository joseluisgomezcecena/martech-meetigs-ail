...
class Area
{
    ...
    private function addArea()
    {
        ...
        {
            $this->errors[] = "Empty Area Name, this field cannot be empty";
        }
        ...
        ) 
        {
            ...
            if (!$this->db_connection->connect_errno) 
            {
                ...
                $area_name = $this->db_connection->real_escape_string(strip_tags($_POST['area_name'], ENT_QUOTES));
               
                $sql = "SELECT * FROM andon_area WHERE area_name = '" . $area_name . "';";
                $query_check_user_name = $this->db_connection->query($sql);

                if ($query_check_user_name->num_rows == 1) 
                {
                    $this->errors[] = "Sorry, that area is already registered.";
                } 
                else 
                {
                    $sql = "INSERT INTO andon_area (site_id, area_name)
                            VALUES('" . $site_id . "', '" . $area_name . "');";
                    $query_new_user_insert = $this->db_connection->query($sql);

                    if ($query_new_user_insert) 
                    {
                        $this->messages[] = "Area registration successful.";
                    } 
                    else 
                    {
